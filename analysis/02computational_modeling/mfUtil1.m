classdef mfUtil1 < handle
    ...
    methods(Static)
        ...
        function model = randomP(model, S)
            ...
            for f = 1:length(fnames)
                ...
                switch param.type
                    ...
                    case 'lognorm'
                        P.(fnames{f}) = exp(param.val(1) + randn(S, 1) .* param.val(2));
                end
            end
            model.P = P;
        end
        ...
        function model = computeEstimates(lik, model, n)
            ...
            sumlik = mfUtil1.logsumexp(lik(inc));
            ...
            for f = 1:length(fnames)
                ...
                [val{f}, ci{f}, samp{f}] = mfUtil1.computeEstimate(param{f}(inc), weights);
        ...
        function model = fit_prior(model)
            ...
            for f = 1:length(fnames)
                ...
                switch model.spec.(fnames{f}).type
                    ...
                    case 'lognorm'
                        [model.spec.(fnames{f}).val(1), model.spec.(fnames{f}).val(2)]= normfit(log(double(samp(~isnan(samp)))));
                end
