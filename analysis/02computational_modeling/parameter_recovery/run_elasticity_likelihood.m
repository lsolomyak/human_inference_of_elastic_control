function [model] = run_elasticity_likelihood(model, N, data, samples, which_models_to_do,model_num)
    S = samples; % number of samples
    for m = which_models_to_do
        improvement = nan;
        while ~(improvement < 1) % repeat until fit stops improving
           
          
            oldbic = model{m}.bic;
   
            for n = 1:N
                model{m} = mfUtil1.randomP(model{m}, S); % sample random parameter values
                
                [lik] = model{m}.lik_func(model{m}.P, data(n)); % compute log-likelihood for each sample
                model{m} = mfUtil1.computeEstimates(lik, model{m}, n); % resample parameter values with each sample weighted by its likelihoods
%                   predicted_content(n, :) = mean(output.predicted_content, 1);
 %                  predicted_excited(n, :) = mean(output.predicted_excited, 1);
                %  expectation(n, :) = mean(output.expectation, 1);
            
            
            end

            
           
    
            % fit prior to resampled parameters
            model{m} = mfUtil1.fit_prior(model{m});
            
            % compute goodness-of-fit measures
            Nparams = 2*length(fieldnames(model{m}.spec)); % number of hyperparameters (assumes 2 hyperparameters per parameter)
        %   Ndatapoints = 19301; % total number of datapoints
            %Ndatapoints = 19301;%numel(vertcat(data(1:N).content))+numel(vertcat(data(1:N).excited)); % total number of datapoints
           %Ndatapoints = numel(vertcat(data(1:N).Reward))+19301;%+numel(vertcat(data(1:N).excited)); % total number of datapoints
           Ndatapoints = numel(vertcat(data(1:N).Reward));%+numel(vertcat(data(1:N).excited)); % total number of datapoints

            model{m}.evidence = sum([model{m}.fit.evidence]); % total evidence
            model{m}.bic = -2*model{m}.evidence + Nparams*log(Ndatapoints); % Bayesian Information Criterion
            improvement = oldbic - model{m}.bic; % compute improvement of fit
        
                fprintf('%s - BIC old: %.2f new: %.2f\n', ...
                    model{m}.name, oldbic, model{m}.bic);
           % fprintf('%s - %s    old: %.2f       new: %.2f      \n', model{m}.name, 'bic', oldbic, model{m}.bic)
        end
    end
end